import { l as lib, a as commonjsGlobal, u as undici, g as getAugmentedNamespace, d as distWeb$1 } from './index-BHzAQa0b.js';
import fs from 'fs';
...
function extractImageUrls$1(body) {
...
function isImageSimilar(hash1, hash2, threshold = 10) {
    return hammingDistance(hash1, hash2) < threshold;
}
// 批量计算图片哈希
async function calculateImageHashes(urls) {
    const results = [];
...
async function findSimilarIssue(newIssue, currentIssueId) {
    ...
    const newIssueImages = extractImageUrls$1(newIssue);
    const hasNewImage = newIssueImages.length > 0;
    const newIssueText = extractText$1(newIssue);
    // 如果新提交包含图片，先计算图片哈希
    let newImageHashes = [];
    if (hasNewImage) {
    ...
    for (let i = 0; i < issues.length; i++) {
        ...
        // 2. URL 精确匹配检查（快速路径）
        if (hasNewImage) {
            const existingImages = extractImageUrls$1(issue.body);
        ...
        const existingText = extractText$1(issue.body);
...
async function callModerationApi(inputs) {
        ...
        try {
            ...
            for (const result of results) {
                ...
                for (const [category, value] of Object.entries(result.categories || {})) {
                    if (value)
                        mergedCategories[category] = true;
                }
            }
...
async function moderateContent(issueNumber, issueBody, dryRun = false, _issuePayload) {
    ...
    if (inputs.length === 0) {
        ...
        if (!dryRun) {
            ...
            await addCommentToIssue(issueNumber, `⚠️内容为空，需要人工审核确认。`);
        }
    ...
    try {
        ...
        if (moderationResult.flagged) {
            const flaggedCategories = Object.keys(moderationResult.categories).filter((category) => moderationResult.categories[category]);
            ...
            if (flaggedCategoriesText.length > 0) {
                if (!dryRun) {
                    ...
                    console.log(`[试运行] 将标记为违规并关闭: ${flaggedCategoriesText.join('、')}`);
                }
                return {
                    type: 'violation',
        ...
        if (!dryRun) {
            await addLabelsToIssue(issueNumber, ['待审']);
            await addCommentToIssue(issueNumber, `⚠️自动审核暂时不可用，内容已提交人工审核。`);
        }
        else {
